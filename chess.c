...
Piece *check_available_piece(wchar_t table[][8], Piece *fp, int turn, int curr_house[2], int in_check) {
  ...
  // TODO: implement movement restriction when in checklib/nfse/betha.rb-
  if (in_check) {
    Piece *king = fp;
...
int **calculate_possible_moves(Piece *p, wchar_t table[][8], int curr_house[2], Piece *fp) {
    ...
    int count = 0;
    int line, col, house_status;

    switch (p->type) {
    case PAWN:
        get_pawn_move(p, possible_moves, fp);
        break;
...
int move_piece(Piece *p, int **possible_moves, wchar_t table[][8], int curr_house[2], Piece *fp) {
    ...
    if (status_house == CAPTURE) {
        Piece *ap = fp;
        while (ap) {
            if (ap->state && ap->line == curr_house[0] && ap->col == curr_house[1]) {
                ap->state = 0;
                break;
            }
            ap = ap->next;
...
int main() {
    ...
    while (!game_over(table)) {
      ...
      while (ok != 1) {
        selected_piece = check_available_piece(table, fp, turn, curr_house, in_check);
